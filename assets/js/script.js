...
devicon.controller('IconListCtrl', function($scope, $http, $compile) {
  ...
  // Get devicon.json from devicon.git submodule
  $http.get(baseUrl + 'devicon.git/devicon.json').success(function(data) {
...
devicon.directive('iconDetails', function ($http, $compile) {
  ...
  return {
    template: '<div class="icon"><article class="icon-detail"><div ng-repeat="svg in icon.svg"><img ng-src="devicon.git/icons/{{icon.name}}/{{icon.name}}-{{svg}}.svg" alt="{{icon.name}}" /></div></article><img ng-src="devicon.git/icons/{{icon.name}}/{{icon.main}}.svg" alt="{{icon.name}}" img-to-svg /></div>',
    replace: true,
    scope: {
      icon: "="
    },
